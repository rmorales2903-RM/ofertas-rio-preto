...
function categoryFor(name) {
  ...
  if (/\b(shampoo|condicionador|sabonete|desodorante|absorvente|creme dental|enxaguante|escova dental|aparelho de barbear|fralda|serum|leave in|tratamento capilar|oleo de queratina|higiene)\b/.test(n)) return 'Higiene e Beleza';
  if (/\b(detergente|amaciante|desinfetante|sabao|lava roupas|limpador|limpa tudo|limpa vidros|desengordurante|tira manchas|papel toalha|agua sanitaria)\b/.test(n)) return 'Limpeza';
...
async function fetchText(url) {
  const r = await fetch(url, {
    headers: { 'user-agent': 'Mozilla/5.0 Promo-Supermercado-Bot/1.0' }
  });
...
function extractPdfLinks(html) {
  ...
  for (const m of html.matchAll(re)) {
    try {
      ...
      if (u.hostname !== 'www.compremixatacado.com.br') continue;
...
function parseValidity(text) {
  ...
  const patterns = [
    /(?:validas?|precos validos|de)\s+(?:para os dias\s+)?(\d{1,2})\s*(?:a|ate|e|à)\s*(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?/i,
    /(?:de|validas? de)\s+(\d{1,2})\/(\d{1,2})\s*(?:a|ate|e|à)\s*(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?/i
  ];

  let m = compact.match(patterns[0]);
  if (m) {
    const d1 = Number(m[1]);
    const d2 = Number(m[2]);
    const mon = Number(m[3]);
    const y = m[4] ? Number(String(m[4]).length === 2 ? `20${m[4]}` : m[4]) : year;
    return { validFrom: `${y}-${String(mon).padStart(2,'0')}-${String(d1).padStart(2,'0')}`, validUntil: `${y}-${String(mon).padStart(2,'0')}-${String(d2).padStart(2,'0')}` };
  }

  m = compact.match(patterns[1]);
  if (m) {
    const d1 = Number(m[1]), mon1 = Number(m[2]), d2 = Number(m[3]), mon2 = Number(m[4]);
    const y = m[5] ? Number(String(m[5]).length === 2 ? `20${m[5]}` : m[5]) : year;
    return { validFrom: `${y}-${String(mon1).padStart(2,'0')}-${String(d1).padStart(2,'0')}`, validUntil: `${y}-${String(mon2).padStart(2,'0')}-${String(d2).padStart(2,'0')}` };
  }
...
function parseOffersFromPdfText(text, sourceUrl) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    const priceMatches = [...line.matchAll(/R\$\s*([0-9.]+,[0-9]{2})/gi)];
    if (!priceMatches.length) continue;

    let namePart = line.slice(0, priceMatches[0].index).trim();
    if (namePart.length < 3 && i > 0) namePart = lines[i - 1];
    if (namePart.length < 3 && i > 1) namePart = `${lines[i - 2]} ${lines[i - 1]}`;

    const name = cleanProductName(namePart);
    if (name.length < 3 || name.length > 180) continue;
    if (/^(cliente clube|cada|kg|oferta|atacado compre mix|orgulho de ser)/i.test(name)) continue;
    ...
    const sameOrNext = `${line} ${lines[i + 1] || ''}`;
    const clubMatch = sameOrNext.match(/cliente\s+clube\s+paga\s+R\$\s*([0-9.]+,[0-9]{2})/i);
    if (clubMatch) {
      const clubPrice = moneyToNumber(clubMatch[1]);
      if (clubPrice && clubPrice > 0) {
        offer = clubPrice;
      ...
      if (p2 && p2 > 0 && p2 <= regular) offer = p2;
    }

    if (!offer || offer <= 0 || offer > 100000) continue;
    if (regular != null && regular < offer) regular = offer;
...
async function ensureStore(sql) {
  const rows = await sql`
    ...
    select sm.id, ct.id, ${STORE_NAME}, ${STORE_ADDRESS}, 'compremix-jd-primavera', true
    from sm, ct
  ...
  try {
    ...
    for (const url of pdfLinks) {
      try {
        ...
        const rows = parseOffersFromPdfText(parsed.text || '', url);
    ...
    if (rows.length > 0) {
      await sql`
        ...
        where store_id=${storeId}
          and raw_payload->>'source'='compremix-pdf'
