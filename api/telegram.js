...
const CATEGORY_BY_TEXT = new Map([
  ...
  ['🥤 bebidas', 'Bebidas'],
  ['bebidas', 'Bebidas'],
  ['🍫 doces e chocolates', 'Doces e Chocolates'],
  ['doces e chocolates', 'Doces e Chocolates'],
  ['🛒 mercearia', 'Mercearia'],
  ['mercearia', 'Mercearia'],
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (text === '/start' || text === 'oi' || text === 'olá' || text === 'ola' || text === 'menu' || text === '📋 menu') {
      await sendTelegram(
        chatId,
        '🛒 <b>Promo Supermercado</b>\n\nEscolha uma categoria abaixo para consultar as melhores ofertas de São José do Rio Preto e Mirassol.',
        MENU
      );
    ...
    if (text === '🏆 melhor preço' || text === 'melhor preço' || text === 'melhor preco') {
      const offers = await sql`
        select distinct on (coalesce(p.normalized_key, o.source_product_name))
               o.id, o.source_product_name, o.source_url, o.regular_price,
               o.offer_price, o.loyalty_required, o.loyalty_label, o.valid_until,
               p.canonical_name, c.name as category,
               s.name as store, sm.name as supermarket
        from offers o
    ...
    await sendTelegram(chatId, 'Não reconheci essa opção. Use os botões do menu ou envie <code>menu</code>.', MENU);
    return json(res, 200, { ok: true });
  } catch (error) {
    console.error(error);
